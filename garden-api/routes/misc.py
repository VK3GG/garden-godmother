...
def quick_suggestions():
    ...
    def _indoor_start_window(plant):
        ...
        trans_raw = plant.get("aus_transplant")
    ...
    with get_db() as db:
        ...
        # Desert ratings: plant_id -> max aus_rating among varieties
        variety_rows = db.execute(
            "SELECT plant_id, MAX(aus_rating) as max_rating FROM varieties GROUP BY plant_id"
        ).fetchall()
        aus_ratings = {r["plant_id"]: r["max_rating"] for r in variety_rows}

        # Best variety name per plant (for display)
        best_variety_rows = db.execute(
            "SELECT plant_id, name, aus_rating FROM varieties WHERE aus_rating >= 4 ORDER BY aus_rating DESC"
        ).fetchall()
        best_variety_name = {}
        for r in best_variety_rows:
            if r["plant_id"] not in best_variety_name:
                best_variety_name[r["plant_id"]] = (r["name"], r["aus_rating"])
        ...
        for plant in all_plants:
            ...
            sow_window = plant.get("aus_sow_outdoor")
            trans_window = plant.get("aus_transplant")
            ...
            max_rating = aus_ratings.get(pid)
...
def list_plant_varieties(plant_id: int):
    ...
    with get_db() as db:
        ...
        if not plant:
            raise HTTPException(404, "Plant not found")
        rows = db.execute(
            "SELECT * FROM varieties WHERE plant_id = ? ORDER BY aus_rating DESC, name",
            (plant_id,),
        ).fetchall()
...
def list_varieties(plant_id: Optional[int] = None, aus_rating_min: Optional[int] = None):
    ...
    with get_db() as db:
        ...
        if aus_rating_min is not None:
            conditions.append("v.aus_rating >= ?")
            params.append(aus_rating_min)
        if conditions:
            query += " WHERE " + " AND ".join(conditions)
        query += " ORDER BY v.aus_rating DESC, p.name, v.name"
...
def recommended_varieties():
    """Top desert-rated varieties (aus_rating >= 4) that are plantable now."""
    today = date.today()
    with get_db() as db:
        # Get all high-rated varieties with their plant data
        rows = db.execute("""
            SELECT v.*, p.name as plant_name, p.category as plant_category,
                   p.aus_sow_outdoor, p.aus_transplant, p.aus_seasons
            FROM varieties v
            JOIN plants p ON v.plant_id = p.id
            WHERE v.aus_rating >= 4
            ORDER BY v.aus_rating DESC, p.name, v.name
        """).fetchall()
        ...
        for row in rows:
            ...
            if actions:
                ...
                for key in ("aus_sow_outdoor", "aus_transplant", "aus_seasons"):
                    r.pop(key, None)
    ...
    with get_db() as db:
        rows = db.execute("""
            SELECT id, name, category, subcategory, days_to_maturity_min,
                   ...
                   heat_tolerance, cold_tolerance, aus_seasons,
                   sow_indoor_weeks_before_transplant, aus_sow_outdoor,
                   aus_transplant, notes
            FROM plants ORDER BY name
        """).fetchall()
        data = []
        for r in rows:
            d = dict(r)
            for key in ("aus_seasons", "aus_sow_outdoor", "aus_transplant"):
